...
Nous disons que $x$ et $y$ sont reliés par un chemin ouvert dans la configuration $\omega$, noté $\smash{x\underset{\omega}{\longleftrightarrow }y}$, s'il existe un chemin qui relie $x,y$ tel que toute arête de ce chemin est ouverte dans la configuration $\omega$. Nous notons aussi $\smash{x\underset{\omega}{\nlongleftrightarrow} y}$ l'événement complémentaire.

Dans ce mémoire, nous étudions en particulier les chemins entre un sommet de $T(l,h)$ et un sommet de $B(l,h)$. Nous notons $\smash{T\underset{\omega}{\longleftrightarrow} B}$ s'il existe un tel chemin dans la configuration $\omega$. 
