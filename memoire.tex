...
$$
Nous posons d'abord la condition aux bords sur les bords horizontaux $T$ et $B$:
...
Soit $t\geqslant 0$, pour un sommet $x$ sur le bord vertical $V(l,h)$, nous déterminons $\Pi^{l,h}_t(x)$ à l'aide une boîte plus grande. Plus précisément, soit $m>l$ entier naturel, nous considérons une boîte $\Lambda(m,h)$ qui contient la boîte $\Lambda(l,h)$, avec une configuration $X_t$ et que pour tout $y\in V(m,h), \Pi^{m,h}_t(y) = null $:
$$\Pi^{l,h}_t(x)=\left\lbrace \begin{array}{cc}
